...
taxon_16s$asv <- rownames(taxon_16s)
  ...
  # Merge all datasets
  merged_genus <- Reduce(function(x, y) full_join(x, y, by = "Genus"),
  ...
  color = colorRampPalette(c("#FFFFFF", "#08306B"))(100)
  ...
  # Reorder the columns
  merged_genus <- merged_genus[, col_order]
 ...
 # Check how the graph looks with abundance cut off of 1%  
  # STEP 1: Make a subset of each dataframe for taxa with >1% mean abundance
  filtered_genus_dfs_1 <- lapply(filtered_genus_dfs, function(df) {
    df %>%
      dplyr::filter(MeanAbun > 0.01)   # 1% = 0.01 relative abundance
  })
  ...
  Heatmap(
    ...
    col = colorRamp2(c(0, max(merged_genus_1)),
                     c("#2C7BB6", "#1A9850"),
    ...
    cluster_columns = FALSE,   # 🔹 keep your specified column order
    # optional: also keep row order if you don't want them clustered
    # cluster_rows = FALSE
  )
  ...
  top_asv_values <- apply(merged_genus, 2, max)
  
  # Convert to percent
  top_asv_percents <- apply(merged_genus, 2, function(col) {
    round(max(col) * 100, 2)
  })
  
  # Combine into a data frame
  top_asvs_df <- data.frame(
    System = colnames(merged_genus),
