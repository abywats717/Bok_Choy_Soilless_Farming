...
# Adjust this number based on your HPC job memory allocation (e.g., 4 cores = ~16GB RAM)
N_THREADS <- 4

# 2. Process in batches to prevent cumulative memory bloat
batch_size <- 20
num_batches <- ceiling(length(fnFs_all) / batch_size)
out_list <- list()

cat("Processing", length(fnFs_all), "samples in", num_batches, "batches...\n")
...
for(i in 1:num_batches) {
  ...
  end_idx <- min(i * batch_size, length(fnFs_all))
  
  # Subset the files for this batch
  batch_fnFs <- fnFs_all[start_idx:end_idx]
  batch_fnRs <- fnRs_all[start_idx:end_idx]
  batch_filtFs <- filtFs_all[start_idx:end_idx]
  batch_filtRs <- filtRs_all[start_idx:end_idx]
  
  # Run filterAndTrim on the batch
  out_batch <- filterAndTrim(
    fwd = batch_fnFs, filt = batch_filtFs,
    rev = batch_fnRs, filt.rev = batch_filtRs,
    truncLen = c(240,240),
    maxN = 0,
    maxEE = c(2,2),
    truncQ = 3,
    rm.phix = TRUE,
    compress = TRUE,
    multithread = N_THREADS # Use limited threads instead of TRUE
  )
  
  out_list[[i]] <- out_batch
  
  # Force garbage collection to free up RAM before the next batch
  gc()
}

# Combine the output matrices back into a single matrix
...
metadata_16s<-read.csv('metadata_all.csv')
...
any(colnames(asv_16s) %in% c("Controls_NTC_Plate4_C4_0", "Controls_NTC_Plate4_D4_2", "Controls_NTC_Plate4_E4_4"))

# Extract the taxonomy table from phyloseq object
taxon.16s<-as.matrix(tax_table(physeq_16s))

# Determine which samples are negative controls
sample_data(physeq_16s)$is.neg <- sample_names(physeq_16s) %in% c("Controls_NTC_Plate4_C4_0", "Controls_NTC_Plate4_D4_2", "Controls_NTC_Plate4_E4_4")
...
# View the list of contaminants
print(contaminant_asvs)

# Filter out ASVs identified as contaminants
physeq_16s <- prune_taxa(!contamdf.prev$contaminant, physeq_16s)

# Remove negative control samples from physeq_16s
physeq_16s <- subset_samples(physeq_16s, !(sample_names(physeq_16s) %in% c("Controls_NTC_Plate4_C4_0", "Controls_NTC_Plate4_D4_2", "Controls_NTC_Plate4_E4_4")))

# Confirm removal
any(sample_names(physeq_16s) %in% c("Controls_NTC_Plate4_C4_0", "Controls_NTC_Plate4_D4_2", "Controls_NTC_Plate4_E4_4"))

# Extract the taxonomy table
taxon_16s <- as.data.frame(tax_table(physeq_16s))

# Convert sample_data to plain data.frame
metadata_16s_df <- as(sample_data(physeq_16s), "data.frame")

# Save metadata table
write.csv(metadata_16s_df, "metadata_16s_clean.csv")

# Save ASV table
write.csv(asv_16s, "ASV_16s_clean.csv")

# Save taxonomy table
write.csv(taxon_16s, "Taxon_16s_clean.csv")
